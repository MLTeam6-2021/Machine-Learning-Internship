...
class WebScraper:
    ...
    def get_topic_created_at(self, topic_soup):
        ...
        """
        created = topic_soup.find('div', class_="topic-body clearfix")
        
        if created is None:
            created_at = str(0)
        else:
            # date the thread was created on
    ...
    def get_topic_replies_nbr(self, topic_soup):
        ...
        """    
        # finding by the 2nd li since there's no class
        replies = topic_soup.find('ul', class_='clearfix')
    ...
    def get_topic_views_nbr(self, topic_soup):
        ...
        """ 
        # finding by the 3rd li since there's no class
        views = topic_soup.find('li', class_='secondary')
        
        if views is None:
            nbr_views = str(0)
        else:
            nbr_views = views.find('span', class_='number heatmap-high').text
    ...
    def get_topic_likes_nbr(self, topic_soup):
        ...
        """ 
        # finding by th 5th li since there's no class
        likes = topic_soup.find('li', class_='secondary')
        
        if likes is None:
            nbr_likes = str(0)
        else:
            nbr_likes = likes.find('span', class_='number')[1].text
    ...
    def runApp(self, BASE_URL, SITE_NAME):
        ...
        # categ_urls = ['https://forums.tapas.io/c/announcements']
        # categ_urls = ['https://forums.tapas.io/c/events-challenges']
        # categ_urls = ['https://forums.tapas.io/c/Off-Topic']
        # categ_urls = ['https://forums.tapas.io/c/art-comics']
        # categ_urls = ['https://forums.tapas.io/c/writing-novels']
        # categ_urls = ['https://forums.tapas.io/c/reviews-feedback']
        # categ_urls = ['https://forums.tapas.io/c/collaborations']
        # categ_urls = ['https://forums.tapas.io/c/questions']
        # categ_urls = ['https://forums.tapas.io/c/answered']
        # categ_urls = ['https://forums.tapas.io/c/tech-support-site-feedback']
        categ_urls = ['https://forums.tapas.io/c/promotions']
        ...
        for categ_url in categ_urls:
            ...
            for topic_link in categ_topic_links:
                categ_topic_urls.append('https://forums.tapas.io' + topic_link['href'])
            ...
            for categ_topic_url in categ_topic_urls:
                ...
                # if (categ_topic_url == "https://forums.tapas.io/t/artist-writer-looking-for-artist-for-an-unpaid-collab/20971"):
                #     break
        ...
        # print(self.topic_df.head(3))
        self.browser.close()
        
        
if __name__=='__main__':
    # Local path to webdriver
    webdriverPath = r'C:\Users\HP\Downloads\geckodriver-v0.29.1-win64\geckodriver.exe'
    
    # Forum to scrape URL    
    BASE_URL = 'https://forums.tapas.io/categories'
    ...
    # data = pd.read_csv('TAPAS_SCRAPED_DATA_20210730134509.csv')
    # data.head(1028)
    # data.info()
